...
def get_mask_from_lengths(lengths):
    ...
    ids = torch.arange(0, max_len, out=torch.cuda.LongTensor(max_len))  # 构造一个从0到max_len-1的Tensor
...
class FlowtronLoss(torch.nn.Module):
    def __init__(self, sigma=1.0, gm_loss=False, gate_loss=True):
        ...
        self.sigma = sigma
        self.gate_criterion = nn.BCEWithLogitsLoss(reduction='none')  # 就按原来的，不取平均
    ...
    def forward(self, model_output, gate_target, lengths):
        ...
        if self.gate_loss:
            ...
            gate_loss = gate_loss.sum() / n_elements  # 在这里取平均
...
class GaussianMixture(torch.nn.Module):
    ...
    def generate_mean(self, n_dimensions, n_components, scale=3):
        ...
        ids = np.random.choice(range(n_dimensions), n_components, replace=False)  # 随机选取几个id
        means = means[ids] * scale
        means = means.transpose(0, 1)
        means = means[None]  # 第一个维度加一维
        return means
...
class Attention(torch.nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None, attn=None,
        ...
        if attn is None:
            ...
            attn = self.v(torch.tanh((queries[:, :, None] + keys[:, None])))    # 通过这个DNN来找相似性
        ...
        output = torch.bmm(attn, values)    # 执行attention的相乘
...
class AR_Back_Step(torch.nn.Module):
    ...
    def forward(self, mel, text, mask, out_lens, attn_prior=None):
        mel = torch.flip(mel, (0,))  # 0轴上反转
        if attn_prior is not None:
            attn_prior = torch.flip(attn_prior, (1,))  # (B, M, T)
        # backwards flow, 把填充的0放到尾部
...
class AR_Step(torch.nn.Module):
    def __init__(self, n_mel_channels, n_speaker_dim, n_text_channels,
        ...
        self.lstm = torch.nn.LSTM(n_hidden + n_attn_channels, n_hidden, n_lstm_layers)  # decoder lstm
    ...
    def forward(self, mel, text, mask, out_lens, attn_prior=None):
        ...
        mel = torch.exp(log_s) * mel + b  # 仿射变换
...
class Flowtron(torch.nn.Module):
    ...
    def infer(self, residual, speaker_ids, text, temperature=1.0,
        ...
        attention_weights = []
        for i, flow in enumerate(reversed(self.flows)):  # 反过来
