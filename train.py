...
def update_params(config, params):
    for param in params:
        ...
        if len(k_split) > 1:
            parent_k = k_split[0]
            cur_param = ['.'.join(k_split[1:])+"="+str(v)]  # 把.复原回去
...
# 把训练好的模型的部分结构（与现模型相同的结构）的参数更新到现模型中
...
def train(n_gpus, rank, output_directory, epochs, optim_algo, learning_rate,
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for batch in train_loader:
            ...
            if rank == 0:
                print("Epoch:{}\titeration:{}:\t{:.9f}".format(epoch, iteration, reduced_loss), flush=True)
...
if __name__ == "__main__":
    ...
    update_params(config, args.params)  # 对一些参数进行改动
    print(config)
